<template>
  <el-radio-group v-bind="getBindValues" v-model="stateValue">
...
const { options: stateOptions, findLabel, findOption } = useOptionQuery<RadioOption>(props)
...
const handleClick = (option: RadioOption) => {
  ...
  const flag = stateLabel.value === label && props.cancelWhenDoubleClick
