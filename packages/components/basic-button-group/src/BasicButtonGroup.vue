  ...
  <div :class="ns.b()">
    <div :class="ns.e('content')">
      ...
      <el-dropdown v-if="hasDropdown" trigger="click" style="margin-left: 10px; cursor: pointer">
        ...
        <template #dropdown>
          <el-dropdown-menu>
            <el-dropdown-item v-for="(button, index) in dropdownButtons" :key="index">
              <component :is="renderButton(button)" />
            </el-dropdown-item>
          </el-dropdown-menu>
        </template>
...
const renderDefaultButton = (button: Button) => {
  ...
  const buttonComponent = h(
    ...
    {
      default: () => text,
      ...(button.icon ? { icon: () => h(button.icon) } : {}),
    },
  )
  ...
  if (button.tooltip && !button.disabledReason) {
    return h(
      ElTooltip,
      { content: button.tooltip, placement: 'top' },
      {
