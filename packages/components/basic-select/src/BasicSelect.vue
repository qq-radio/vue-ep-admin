...
const isMultiple = computed(() => props.hasCheckAll || props.multiple)
...
watch(
  () => props.modelValue,
  () => {
    const isMultipleAndEmpty = isMultiple.value && isEmpty(props.modelValue)
    stateValue.value = isMultipleAndEmpty ? [] : props.modelValue
