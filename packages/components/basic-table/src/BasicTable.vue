<template>
  <div :class="ns.b()">
    ...
    <div :class="ns.e('operation')">
      <div :class="ns.e('operation-left')">
        <slot name="operation-before" />
        <BasicButtonGroup :buttons="getProps.operations" @success="reQuery" />
        <slot name="operation-after" />
      </div>
      <div :class="ns.e('operation-right')">
        <ColumnRefresh @refresh="reQuery" />
        <ColumnDensity :size="getBindValues.size" @density-change="setTableSize" />
        <ColumnSetting :schemas="schemas" @change="onColumnSettingChange" />
      </div>
    </div>
    <div :class="ns.e('body')">
      <el-table
        ...
        <template #default>
          <slot name="default">
            ...
            <template v-for="schema in getTableSchemas" :key="schema.prop">
              <el-table-column
                v-if="schema && schema.headerIsChecked"
...
import type { BasicTableProps, BasicTableEmits, TableMethods, TableSize } from './types'
...
import ColumnRefresh from './components/ColumnRefresh.vue'
import ColumnDensity from './components/ColumnDensity.vue'
...
const getBindValues = computed(() => ({
  ...
  size: getProps.value.size || 'default',
}))

const setTableSize = (v: TableSize) => {
  setTableProps({ size: v })
}
